...
function Detail() {
  ...
  return (
    <>
    <section className='app__Detail container mt-16 md:mt-20 mx-auto max-w-7xl px-2 sm:px-4 lg:px-12 xl:max-w-screen-xl ' id='detail'>
          ...
          <ul className="px-2 lg:px-8 mx-auto mt-10 sm:mt-2 grid grid-cols-2 gap-x-4 gap-y-8 pt-6 sm:pt-10 lg:mx-0 lg:max-w-none lg:grid-cols-4">
            {detailContent.map((item,index) => (
              <li key={index} className="flex max-w-md flex-col items-start justify-between">
                <img src={item.icon} alt={item.alt} className='max-w-12'/>
              ...
              <button
                type="button"
                className="inline-flex items-center rounded-xl bg-white px-5 py-3 text-sm font-semibold text-indigo-600 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-100 my-3 transition-all ease-in duration-100"
              >
                Learn more
              </button>
            </li>
